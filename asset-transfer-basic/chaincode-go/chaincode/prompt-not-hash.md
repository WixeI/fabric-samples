...
The function GenerateEncryptionKey will populate the private collection key "encryption_key" with an encryption key that will be used in the function GenerateOrgHash. For now, the value should just be the mspId of the company executing the function, just store that value. The private collection is the implicit private collection that fabric generates for every organization. It can be accessed by writing code in the same format as shown below:
...
The function GenerateOrgHash is about encryption, but for now, all it does is retrieve and return the value of the private collection "encryption_key" (which, for now, is the mspId value).
...
There should be a function called IsOwner. It will receive a string as an argument. You should get the encryption_key private collection value of your private collection,and compare with the string received. If it's the same, return true, for you are the owner. If it doesn't match, return false, for you're not the owner.
...
Now, for the most important part: the ability to start trades. Trades work the following way: one company, wanting to buy a bond they saw on the Bonds list in the ledger, will run the function CreateTrade and put a cusip as argument. A new item in the DirectTrades list will be created, of type DirectTrade. Another argument of the function is the OriginalFace, that is gonna fill the field with the same name of the DirectTrade struct. The BidPrice should also be an argument and behave in the same way. The State field should be "Open". The answers field should be an empty array. The DirectTradeID should be generated on the spot. The BidderHash should use the GenerateOrgHash function value. The timestamp should be generated on the spot for the current time.

There should be a function called GetYourDirectTrades, that returns a list of all DirectTrades that you are the owner of. Use the IsOwner function with the BidderHash of each DirectTrade to check ownership.

There should be a function called CloseDirectTrade, that receives a DirectTradeID string. It changes the status of a DirectTrade object to "Closed", but only if you are the owner of that DirectTrade object (check by the BidderHash with isOwner)
